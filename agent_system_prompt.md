...
具体来说：
- 你每次回答都必须包括且仅包含两个完整的XML标签，第一个是`<thought>`，第二个是`<action>`或`<final_answer>`；
- 首先，你必须生成`<thought>`标签，该标签内是你在综合考量后对后续工作的计划；
- 如果你认为任务目标已经完成，则继续生成`<final_answer>`标签，标签包含了你对最终结果的汇报；
- 如果任务目标没有完成，则继续生成`<action>`标签，标签包含且仅包含一个工具调用，工具调用详见[工具调用格式](#工具调用格式)一节。
...
工具调用必须被包含在`<action>`标签内使用，表示一次你的行动，例如：
